...
import { CORE_BRIDGE_PID, LOCALHOST, PAYER_KEYPAIR, boilerPlateReduction, range } from "./tests/helpers";
import * as wormhole from "@certusone/wormhole-sdk/lib/cjs/solana/wormhole";
import * as MyWormholeProject from "./tests/sdk";
import { deriveAddress } from "@certusone/wormhole-sdk/lib/cjs/solana";
import * as mock from "@certusone/wormhole-sdk/lib/cjs/mock";

const FUZZ_TEST_ITERATIONS = 64;
const HELLO_WORLD_PID = new PublicKey("F1tEp6JUoq255o2nWr1nEZJcQtv5dKWryLe6scHLqmPd");
...
describe(" 1: Hello World", function() {
  ...
  const realConfig = MyWormholeProject.deriveConfigKey(HELLO_WORLD_PID);
  const realForeignEmitter =
    MyWormholeProject.deriveForeignEmitterKey(HELLO_WORLD_PID, realForeignEmitterChain);
  const program = MyWormholeProject.createMyWormholeProjectProgramInterface(connection, HELLO_WORLD_PID);
  ...
  describe("Initialize Program", function() {
    const wormholeCpi = wormhole.getPostMessageCpiAccounts(
      HELLO_WORLD_PID,
      CORE_BRIDGE_PID,
      payer.publicKey,
    ...
    console.log("Real config is",realConfig.toString())

    const createInitializeIx = () =>
      MyWormholeProject.createInitializeInstruction(
        connection,
        HELLO_WORLD_PID,
        payer.publicKey,
        CORE_BRIDGE_PID
    ...
    it("Finally Set Up Program", async function() {
      ...
      // verify account data
      const configData = await MyWormholeProject.getConfigData(connection, HELLO_WORLD_PID);
  ...
  describe("Register Foreign Emitter", function() {
    ...
    .forEach(emitterChain =>
      it(`Cannot Register Chain ID == ${emitterChain}`, async function() {
        await expectIxToFailWithError(
          await MyWormholeProject.createRegisterForeignEmitterInstruction(
            connection,
            HELLO_WORLD_PID,
            payer.publicKey,
            emitterChain,
    ...
    it("Cannot Register Emitter Address Length != 32", async function() {
      await expectIxToFailWithError(
        MyWormholeProject.createRegisterForeignEmitterInstruction(
          connection,
          HELLO_WORLD_PID,
          payer.publicKey,
          realForeignEmitterChain,
    ...
    .forEach((emitterAddress) =>
      ...
      async function() {
        ...
        const {chain, address} = 
          await MyWormholeProject.getForeignEmitterData(
            connection,
            HELLO_WORLD_PID,
            realForeignEmitterChain
          );
  ...
  describe("Send Message", function() {
    it("Cannot Send Message With Length > 512", async function() {
      ...
      await expectIxToFailWithError(
        await MyWormholeProject.createSendMessageInstruction(
          connection,
          HELLO_WORLD_PID,
          payer.publicKey,
          CORE_BRIDGE_PID,
    ...
    it("Finally Send Message", async function() {
      ...
      await expectIxToSucceed(
        MyWormholeProject.createSendMessageInstruction(
          connection,
          HELLO_WORLD_PID,
          payer.publicKey,
          CORE_BRIDGE_PID,
      ...
      const {payload} =
        (await wormhole.getPostedMessage(
          connection,
          MyWormholeProject.deriveWormholeMessageKey(HELLO_WORLD_PID, sequence)
        )).message;
  ...
  describe("Receive Message", function() {
    ...
    const createAndReceiveIx = (signedMsg: Buffer) =>
      MyWormholeProject.createReceiveMessageInstruction(
        connection,
        HELLO_WORLD_PID,
        payer.publicKey,
        CORE_BRIDGE_PID,
    ...
    it("Finally Receive Message", async function() {
      ...
      const received = await MyWormholeProject.getReceivedData(
        connection,
        HELLO_WORLD_PID,
